...
impl RoleRepository {
    ...
    /// 检查角色是否拥有特定权限
    pub async fn role_has_permission(&self, role_id: Uuid, resource: &str, action: &str) -> Result<bool, AppError> {
    ...
    /// 从角色移除权限
    pub async fn remove_permission_from_role(&self, role_id: Uuid, permission_id: Uuid) -> Result<bool, AppError> {
    ...
    /// 获取用户的角色绑定
    pub async fn get_user_role_bindings(&self, user_id: Uuid) -> Result<Vec<RoleBinding>, AppError> {
